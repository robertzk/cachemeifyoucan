...
describe("update_cache_salt", {
  ...
  db_test_that('updating_cache_salt creates new shards', {
    ...
    takes_less_than(1)(old_df <- old_cache_fn(key = ids, model_version = model_version, locale = locale))
    ...
    takes_less_than(1)(new_df <- new_cache_fn(key = ids, model_version = model_version, locale = locale))
